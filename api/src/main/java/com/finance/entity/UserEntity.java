...
public class UserEntity {
    ...
    // In production this would default to false until the user clicks a
    // verification link.
    ...
    // updatable = false — Hibernate never includes created_at in an UPDATE
    // statement.
    ...
    // insertable = false, updatable = false — Hibernate never touches updated_at at
    // all.
    // The DB trigger trg_users_set_updated_at owns this field entirely.
    // This ensures updated_at is accurate even if someone runs SQL directly against
    // the DB,
    ...
    // updated_at is intentionally absent — the DB DEFAULT NOW() on the column
    // handles
    ...
    public void setFailedLoginCount(int n) {
