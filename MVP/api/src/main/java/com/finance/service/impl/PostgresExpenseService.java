...
public class PostgresExpenseService implements ExpenseService {
    ...
    public ExpensePage queryExpenses(UUID userId, ExpenseQuery q) {
        ...
        if (!q.includeDeleted()) sql.append(" AND e.deleted_at IS NULL");
        if (q.dateFrom() != null) { sql.append(" AND e.expense_date >= :dateFrom"); params.put("dateFrom", q.dateFrom()); }
        if (q.dateTo() != null)   { sql.append(" AND e.expense_date <= :dateTo");   params.put("dateTo", q.dateTo()); }
        if (q.merchantName() != null) { sql.append(" AND e.merchant_name ILIKE :merchant"); params.put("merchant", "%" + q.merchantName() + "%"); }
        if (q.minAmount() != null)    { sql.append(" AND e.amount >= :minAmt"); params.put("minAmt", q.minAmount()); }
        if (q.maxAmount() != null)    { sql.append(" AND e.amount <= :maxAmt"); params.put("maxAmt", q.maxAmount()); }
        if (q.paymentMethod() != null) { sql.append(" AND e.payment_method = :pm"); params.put("pm", q.paymentMethod()); }
        if (q.bankAccountId() != null) { sql.append(" AND e.bank_account_id = :baid"); params.put("baid", q.bankAccountId()); }
        if (q.source() != null) { sql.append(" AND e.source = :source"); params.put("source", q.source()); }
        if (q.categories() != null && !q.categories().isEmpty()) {
            sql.append(" AND c.name = ANY(:cats)");
        ...
        String countSql = sql.toString()
                .replace("SELECT DISTINCT e.*", "SELECT COUNT(DISTINCT e.id)")
                .replaceAll("ORDER BY.*", "");
        ...
        Query dataQ = entityManager.createNativeQuery(sql.toString(), ExpenseEntity.class);
