...
- **S1 — Split RLS bypass into a dedicated connection pool. ✓ Shipped (with Option-A pivot for Render deploy).** Two Hikari pools (`appDataSource` connecting as `expense_app`, `setupDataSource`). V20 revokes `expense_app`'s membership in `expense_setup`, severing the SQL-injection escalation path. The three pre-auth methods (`register`, `login`, `setupNewUser`) route through `setupJdbcTemplate`; `RoleElevationService` is deleted. The security boundary is regression-tested by `PoolIsolationIntegrationTest`. Under the Option-A pivot, the setup pool connects as the table-owner role rather than a BYPASSRLS `expense_setup` role — managed Postgres refuses to grant BYPASSRLS. See [ADR-0011](decisions/0011-three-layer-rls-defence.md) for the trade-off and reversion path.
...
- **S6 — Integration suite enabled + v2.0 correctness pass. ✓ Shipped (2026-06-04).** The full Testcontainers suite had never run (`useJUnitPlatform()` was missing, so Gradle's JUnit-4 runner found zero JUnit-5 tests). Enabling it (144 tests, now green in CI with `--continue`) exposed eight production bugs that had shipped untested — most importantly that the v2.0 RLS policies were `AS RESTRICTIVE` with no permissive policy (= default-deny), which made delegation and CSV import fully non-functional over the app pool. Fixes: V31 (drop `FORCE` so owner-bypass works), V32 (RLS fail-closed on empty context), V33 (SECURITY DEFINER cross-user lookups for delegation), V34 (`PERMISSIVE` policies), a `REQUIRES_NEW` `LoginFailureRecorder` so account lockout actually persists, `@Transactional` on the CSV read paths, `JwtService` validating against its injected clock, and several Spring-wiring fixes. See [testing-strategy.md](architecture/testing-strategy.md) for the full catalogue and the test patterns that prevent recurrence.
