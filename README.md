...
# Remove Me
Phase 1
1. Where to store the data and what are the general access patterns
1. What functions should be available? Login, log out, add expense, delete expense, and etc
1. How to perform those actions, backend API, different modules packaged up, or any other options Once those are determine then we
---
Phase 2
1. Remaining API contract
1. Design patterns and class structure of how the API end points work. [This should include why other options are either not suitable or no different]
1. Data model, restrictions, triggers, and indexes
1. Full C4 diagram
1. Unit testing
---
**Updated version 1.0 table list**
1. Identity and access — users, access grants, revoked tokens, sudo tokens, bank accounts.
1. Expense management — categories, expenses, expense categories junction, idempotency keys.
1. Financial intelligence — targets, target categories junction.
1. Infrastructure — partition registry, materialised views.
